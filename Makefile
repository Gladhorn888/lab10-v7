...
sanitize: clean
	$(CC) $(CFLAGS) $(SANITIZER_FLAGS) -g -c $(SOURCES) -o src/stack.o
	$(CC) $(SANITIZER_FLAGS) -shared -o $(TARGET_LIB) src/stack.o $(LDFLAGS)
	$(CC) $(CFLAGS) $(SANITIZER_FLAGS) -g test/test.c -L. -lstack -o $(TEST_BIN)
	LD_LIBRARY_PATH=. ./$(TEST_BIN)

analyze:
	mkdir -p reports
	$(ANALYZER) src/stack.c -- $(CFLAGS) 2>&1 | tee reports/clang-tidy.log
